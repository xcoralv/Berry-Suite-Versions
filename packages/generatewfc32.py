...
class WfcGenerator:
    ...
    def run(self):
        ...
        self.tmp_dir = f"{os.getcwd()}/tmp_rank{self.rank}"
        ...
        if isinstance(self.nk_points, range):
            ...
            processes = np.array_split(self.nk_points, self.size)[self.rank]
            print('THIS IS PROCESSES', processes)
            print('nk_points',self.nk_points)
            print('size',self.size)
            print('rank',self.rank)
            for nk in processes:
                print('        THIS IS NK IN PROCESSES:', nk)
